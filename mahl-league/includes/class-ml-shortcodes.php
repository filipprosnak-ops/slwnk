...
class ML_Shortcodes {
	...
	public static function render_teams(): string {
		if ( ! self::is_ready() ) {
			return '<p>' . esc_html__( 'League module is temporarily unavailable.', 'mahl-league' ) . '</p>';
		}

		$teams = get_posts(array('post_type'=>'ml_team','post_status'=>'publish','posts_per_page'=>-1,'orderby'=>'title','order'=>'ASC'));
		if ( empty( $teams ) ) {
			return '<p>' . esc_html__( 'No teams available.', 'mahl-league' ) . '</p>';
		}
	...
	public static function render_matches( array $atts ): string {
		...
		if ( empty( $matches ) ) {
			return '<p>' . esc_html__( 'No matches available.', 'mahl-league' ) . '</p>';
		}
	...
	public static function render_standings(): string {
		...
		if ( empty( $standings ) ) {
			return '<p>' . esc_html__( 'Standings are not available for selected context.', 'mahl-league' ) . '</p>';
		}
	...
	public static function render_stats(): string {
		...
		if ( empty( $stats ) ) {
			return '<p>' . esc_html__( 'Statistics are not available for selected context.', 'mahl-league' ) . '</p>';
		}
		...
		return '<p>' . esc_html( sprintf( __( 'Cached stats loaded: %1$d teams, %2$d players.', 'mahl-league' ), $team_count, $player_count ) ) . '</p>';
	}
	...
	public static function render_playoffs(): string {
		...
		if ( empty( $matches ) ) {
			return '<p>' . esc_html__( 'No playoff matches available.', 'mahl-league' ) . '</p>';
		}
